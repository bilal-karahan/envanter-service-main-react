...
class Category extends CI_Controller {
    ...
    public function index(){
        sleep(2);
        $categories = $this->crud_model->get(array('status!='=>'2'),'inserted_date DESC','tge_kategoriler',$this->db);
    ...
    public function add(){
        ...
        $form_kural = array(
            array(
                'field' => 'title',
                'label' => 'Ad',
                'rules' => 'trim|required|max_length[10]|min_length[3]'
            )
        );
        ...
        if(!$this->form_validation->run()){
            ...
            echo json_encode(array(
                "result" => "error",
                "title" => "İşlem Sonucu",
                "valid" => $this->form_validation->error_array()
            ));
        }
    ...
    public function delete(){
        ...
        $category = $this->crud_model->get_single(array("id"=>$id),'','tge_kategoriler');
        if(!$category){
            // İstek atılan yerde karşıla
            echo json_encode(array(
                "result" => "error",
